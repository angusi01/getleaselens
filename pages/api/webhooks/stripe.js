...
export default async function handler(req, res) {
    ...
    if (error || !purchase)
        return res.status(200).json({ received: true });
    if (['paid', 'processing', 'complete', 'error'].includes(purchase.status))
        return res.status(200).json({ received: true });
    await supabase.from('purchases').update({ status: 'processing', email }).eq('id', purchaseId).eq('status', 'pending');
